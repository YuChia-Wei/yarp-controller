...
{
    private static readonly Counter<long> TotalConnections = GatewayMeter.Meter.CreateCounter<long>(
        "yarp_kestrel_total_connections",
        description: "Number of incoming connections opened"
    );

    private static readonly Counter<long> TotalTlsHandshakes = GatewayMeter.Meter.CreateCounter<long>(
        "yarp_kestrel_total_tls_Handshakes",
        description: "Number of TLS handshakes started"
    );

    private static readonly ObservableGauge<double> CurrentTlsHandshakes = GatewayMeter.Meter.CreateObservableGauge<double>(
    ...
    private static readonly Counter<long> FailedTlsHandshakes = GatewayMeter.Meter.CreateCounter<long>(
        "yarp_kestrel_failed_tls_handshakes",
        description: "Number of TLS handshakes that failed"
    );

    private static readonly ObservableGauge<double> CurrentConnections = GatewayMeter.Meter.CreateObservableGauge<double>(
    ...
    private static readonly ObservableGauge<double> ConnectionQueueLength = GatewayMeter.Meter.CreateObservableGauge<double>(
        "yarp_kestrel_connection_queue_length",
        description: "Number of connections on the queue.",
        observeValue: () => _connectionQueueLength
    );

    private static readonly ObservableGauge<double> RequestQueueLength = GatewayMeter.Meter.CreateObservableGauge<double>(
