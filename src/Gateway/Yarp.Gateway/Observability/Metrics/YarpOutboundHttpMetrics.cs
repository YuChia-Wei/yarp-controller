...
{
    private static readonly Counter<long> OutboundRequestsStarted = GatewayMeter.Meter.CreateCounter<long>(
        "yarp_outbound_http_requests_started",
        description: "Number of outbound requests initiated by the proxy"
    );

    private static readonly Counter<long> OutboundRequestsFailed = GatewayMeter.Meter.CreateCounter<long>(
        "yarp_outbound_http_requests_failed",
        description: "Number of outbound requests failed"
    );

    private static readonly ObservableGauge<double> OutboundCurrentRequests = GatewayMeter.Meter.CreateObservableGauge<double>(
    ...
    private static readonly ObservableGauge<double> OutboundCurrentHttp11Connections = GatewayMeter.Meter.CreateObservableGauge<double>(
    ...
    private static readonly ObservableGauge<double> OutboundCurrentHttp20Connections = GatewayMeter.Meter.CreateObservableGauge<double>(
    ...
    private static readonly Histogram<double> OutboundHttp11RequestQueueDuration = GatewayMeter.Meter.CreateHistogram<double>(
    ...
    private static readonly Histogram<double> OutboundHttp20RequestQueueDuration = GatewayMeter.Meter.CreateHistogram<double>(
