...
internal class ConfigurationConfigProvider : IProxyConfigProvider, IDisposable
{
    ...
    public IProxyConfig GetConfig()
    {
        ...
        {
            this._subscription = ChangeToken.OnChange(this._configuration.GetReloadToken, this.UpdateSnapshot);
            this.UpdateSnapshot();
        }
    ...
    private static HttpClientConfig? CreateHttpClientConfig(IConfigurationSection section)
    {
        ...
        {
            foreach (var protocolConfig in sslProtocolsSection.GetChildren().Select(s => Enum.Parse<SslProtocols>(s.Value!, true)))
            {
    ...
    private void UpdateSnapshot()
    {
        ...
        {
            Log.LoadData(this._logger);
            ConfigurationSnapshot newSnapshot;
            try
            {
                newSnapshot = new ConfigurationSnapshot();

                foreach (var section in this._configuration.GetSection("Clusters").GetChildren())
                {
                    newSnapshot.Clusters.Add(this.CreateCluster(section));
                }

                foreach (var section in this._configuration.GetSection("Routes").GetChildren())
                {
            ...
            {
                Log.ConfigurationDataConversionFailed(this._logger, ex);
            ...
            var oldToken = this._changeToken;
            this._changeToken = new CancellationTokenSource();
            newSnapshot.ChangeToken = new CancellationChangeToken(this._changeToken.Token);
            ...
            {
                Log.ErrorSignalingChange(this._logger, ex);
            }
        }
    }
