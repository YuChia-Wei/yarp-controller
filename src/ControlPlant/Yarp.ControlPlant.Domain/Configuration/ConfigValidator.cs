...
using Yarp.ReverseProxy.ControlPlant.Entity.Health;
using Yarp.ReverseProxy.ControlPlant.Entity.LoadBalancing;
using Yarp.ReverseProxy.ControlPlant.Entity.SessionAffinity;
...
internal class ConfigValidator : IConfigValidator
{
    ...
    {
        this._authorizationPolicyProvider = authorizationPolicyProvider ?? throw new ArgumentNullException(nameof(authorizationPolicyProvider));
        this._rateLimiterPolicyProvider = rateLimiterPolicyProvider ?? throw new ArgumentNullException(nameof(rateLimiterPolicyProvider));
        this._corsPolicyProvider = corsPolicyProvider ?? throw new ArgumentNullException(nameof(corsPolicyProvider));
        this._loadBalancingPolicies =
            loadBalancingPolicies?.ToDictionaryByUniqueId(p => p.Name) ?? throw new ArgumentNullException(nameof(loadBalancingPolicies));
        this._affinityFailurePolicies = affinityFailurePolicies?.ToDictionaryByUniqueId(p => p.Name) ??
                                        throw new ArgumentNullException(nameof(affinityFailurePolicies));
        this._availableDestinationsPolicies = availableDestinationsPolicies?.ToDictionaryByUniqueId(p => p.Name) ??
                                              throw new ArgumentNullException(nameof(availableDestinationsPolicies));
        this._activeHealthCheckPolicies = activeHealthCheckPolicies?.ToDictionaryByUniqueId(p => p.Name) ??
                                          throw new ArgumentNullException(nameof(activeHealthCheckPolicies));
        this._passiveHealthCheckPolicies = passiveHealthCheckPolicies?.ToDictionaryByUniqueId(p => p.Name) ??
                                           throw new ArgumentNullException(nameof(passiveHealthCheckPolicies));
    ...
    public async ValueTask<IList<Exception>> ValidateRouteAsync(RouteConfig route)
    {
        ...
        await this.ValidateAuthorizationPolicyAsync(errors, route.AuthorizationPolicy, route.RouteId);
#if NET7_0_OR_GREATER
        await this.ValidateRateLimiterPolicyAsync(errors, route.RateLimiterPolicy, route.RouteId);
#endif
        await this.ValidateCorsPolicyAsync(errors, route.CorsPolicy, route.RouteId);

        if (route.Match is null)
        {
            errors.Add(new ArgumentException(
                           $"Route '{route.RouteId}' did not set any match criteria, it requires Hosts or Path specified. Set the Path to '/{{**catchall}}' to match all requests."));
            return errors;
        }
        ...
        {
            errors.Add(new ArgumentException(
                           $"Route '{route.RouteId}' requires Hosts or Path specified. Set the Path to '/{{**catchall}}' to match all requests."));
        }
    ...
    public ValueTask<IList<Exception>> ValidateClusterAsync(ClusterConfig cluster)
    {
        ...
        this.ValidateLoadBalancing(errors, cluster);
        this.ValidateSessionAffinity(errors, cluster);
        ValidateProxyHttpClient(errors, cluster);
        this.ValidateProxyHttpRequest(errors, cluster);
        this.ValidateHealthChecks(errors, cluster);
    ...
    private void ValidateActiveHealthCheck(IList<Exception> errors, ClusterConfig cluster)
    {
        ...
        if (!this._activeHealthCheckPolicies.ContainsKey(policy))
        {
            errors.Add(new ArgumentException(
                           $"No matching {nameof(IActiveHealthCheckPolicy)} found for the active health check policy name '{policy}' set on the cluster '{cluster.ClusterId}'."));
        }
    ...
    private async ValueTask ValidateAuthorizationPolicyAsync(IList<Exception> errors, string? authorizationPolicyName, string routeId)
    {
        ...
        {
            var policy = await this._authorizationPolicyProvider.GetPolicyAsync(authorizationPolicyName);
    ...
    private async ValueTask ValidateCorsPolicyAsync(IList<Exception> errors, string? corsPolicyName, string routeId)
    {
        ...
        if (string.Equals(CorsConstants.Default, corsPolicyName, StringComparison.OrdinalIgnoreCase))
        {
            ...
            var policy = await this._corsPolicyProvider.GetPolicyAsync(dummyHttpContext, corsPolicyName);
            if (policy is not null)
            {
                errors.Add(new ArgumentException(
    ...
    private static void ValidateHeaders(List<Exception> errors, IReadOnlyList<RouteHeader>? headers, string routeId)
    {
        ...
        foreach (var header in headers)
        {
            ...
            {
                errors.Add(new ArgumentException(
                               $"Header values were set when using mode '{header.Mode}' on route header '{header.Name}' for route '{routeId}'."));
            }
        }
    }
    ...
    private void ValidateHealthChecks(IList<Exception> errors, ClusterConfig cluster)
    {
        ...
        if (!this._availableDestinationsPolicies.ContainsKey(availableDestinationsPolicy))
        {
            errors.Add(new ArgumentException(
                           $"No matching {nameof(IAvailableDestinationsPolicy)} found for the available destinations policy '{availableDestinationsPolicy}' set on the cluster.'{cluster.ClusterId}'."));
        }

        this.ValidateActiveHealthCheck(errors, cluster);
        this.ValidatePassiveHealthCheck(errors, cluster);
    }
    ...
    private void ValidateLoadBalancing(IList<Exception> errors, ClusterConfig cluster)
    {
        ...
        if (!this._loadBalancingPolicies.ContainsKey(loadBalancingPolicy))
        {
            errors.Add(new ArgumentException(
                           $"No matching {nameof(ILoadBalancingPolicy)} found for the load balancing policy '{loadBalancingPolicy}' set on the cluster '{cluster.ClusterId}'."));
        }
    }
    ...
    private void ValidatePassiveHealthCheck(IList<Exception> errors, ClusterConfig cluster)
    {
        ...
        if (!this._passiveHealthCheckPolicies.ContainsKey(policy))
        {
            errors.Add(new ArgumentException(
                           $"No matching {nameof(IPassiveHealthCheckPolicy)} found for the passive health check policy name '{policy}' set on the cluster '{cluster.ClusterId}'."));
        }
    ...
    private void ValidateProxyHttpRequest(IList<Exception> errors, ClusterConfig cluster)
    {
        ...
        {
            errors.Add(new ArgumentException(
                           $"Outgoing request version '{cluster.HttpRequest.Version}' is not any of supported HTTP versions (1.0, 1.1, 2 and 3)."));
        }

        if (cluster.HttpRequest.Version == HttpVersion.Version10)
        {
            Log.Http10Version(this._logger);
        }
    }
    ...
    private static void ValidateQueryParameters(List<Exception> errors, IReadOnlyList<RouteQueryParameter>? queryparams, string routeId)
    {
        ...
        foreach (var queryparam in queryparams)
        {
            ...
            {
                errors.Add(new ArgumentException(
                               $"Query parameter values where set when using mode '{nameof(QueryParameterMatchMode.Exists)}' on route query parameter '{queryparam.Name}' for route '{routeId}'."));
            }
        }
    }
    ...
    private async ValueTask ValidateRateLimiterPolicyAsync(IList<Exception> errors, string? rateLimiterPolicyName, string routeId)
    {
        ...
        try
        {
            var policy = await this._rateLimiterPolicyProvider.GetPolicyAsync(rateLimiterPolicyName);
    ...
    private void ValidateSessionAffinity(IList<Exception> errors, ClusterConfig cluster)
    {
        ...
        if (!this._affinityFailurePolicies.ContainsKey(affinityFailurePolicy))
        {
            errors.Add(new ArgumentException(
                           $"No matching {nameof(IAffinityFailurePolicy)} found for the affinity failure policy name '{affinityFailurePolicy}' set on the cluster '{cluster.ClusterId}'."));
        }
