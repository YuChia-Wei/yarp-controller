...
{
    private volatile ClusterDestinationsState _destinationsState =
        new ClusterDestinationsState(Array.Empty<DestinationState>(), Array.Empty<DestinationState>());

    private volatile ClusterModel _model = default!; // Initialized right after construction.

    /// <summary>
    /// Creates a new instance. This constructor is for tests and infrastructure, this type is normally constructed by the
    /// configuration
    /// loading infrastructure.
    /// </summary>
    public ClusterState(string clusterId)
    {
        this.ClusterId = clusterId ?? throw new ArgumentNullException(nameof(clusterId));
    }
    ...
    {
        get => this._destinationsState;
        set => this._destinationsState = value ?? throw new ArgumentNullException(nameof(value));
    }
    ...
    /// <summary>
    /// Tracks changes to the cluster configuration for use with rebuilding dependent endpoints. Destination changes do not
