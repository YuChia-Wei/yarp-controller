...
public class CreateClusterCommandHandler : IRequestHandler<CreateClusterCommand, ExecuteResult>
{
    private readonly IClusterCreator _clusterCreator;
    private readonly IClusterExistChecker _clusterExistChecker;

    public CreateClusterCommandHandler(IClusterCreator clusterCreator, IClusterExistChecker clusterExistChecker)
    {
        this._clusterCreator = clusterCreator;
        this._clusterExistChecker = clusterExistChecker;
    }

    public async ValueTask<ExecuteResult> Handle(CreateClusterCommand request, CancellationToken cancellationToken)
    {
